...
function About() {
    ...
    const [selectedTeamName,setSelectedTeamName] = useState(0)
    return (
        <Wrapper>
            ...
            <div className="p-3 w-full flex justify-center items-center flex-col gap-8 font-lostfish">
                ...
                backdropFilter:" blur(3.5px)"}} className="w-[90%] md:w-[50%] rounded-md px-4 py-2 text-xl" value={selectedTeamName} onChange={(e)=>setSelectedTeamName(e.target.value)}>
                    {
                        teamNames.map((teamName,key)=>(
                            <option className="text-black" value={key}>{teamName}</option>
                        ))
                    }
                </select>
